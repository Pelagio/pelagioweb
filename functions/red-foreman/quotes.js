module.exports = [
  [
    {
      character: "Red Forman",
      quote:
        "When my time comes I want to be buried face down. That way whoever doesn't like me can kiss my ass."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "What have I said about comparing your sister to the Devil?"
    },
    { character: "Eric", quote: "That it's offensive to the Devil?" }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to Eric]  So, this is how an immature, engaged, high school dumbass, with no car, no job, and no money trims the hedges."
    },
    {
      character: "Steven Hyde",
      quote: "That was like eight burns in one sentence."
    },
    {
      character: "Donna Pinciotti",
      quote: "An octo-burn. Let's get outta here."
    }
  ],
  [
    { character: "Fez", quote: "You don't like me because I'm not from here." },
    {
      character: "Red",
      quote:
        "This has nothing to do with you being a foreigner. This is about you taking advantage of my daughter like a sneaky foreigner!"
    }
  ],
  [
    { character: "Red", quote: "Nothing around this house is cheap." },
    { character: "Eric", quote: "Except for Laurie." },
    { character: "Laurie Forman", quote: "I am not cheap!" },
    { character: "Eric", quote: "Free, whatever." }
  ],
  [
    {
      character: "Red Forman",
      quote: "[Red and Eric are in the car almost late for work at Pricemart]"
    },
    { character: null, quote: "[to Eric]" },
    {
      character: "Red Forman",
      quote: "You need to grow up and learn some responsibility."
    },
    { character: "Eric", quote: "I love you too, Dad." },
    { character: "Red Forman", quote: "What? Stop being weird." },
    { character: "Eric", quote: "Thanks." }
  ],
  [
    { character: "Eric", quote: "[badly hungover]  My head hurts." },
    {
      character: "Red Forman",
      quote: "That's your brain trying to comprehend its own stupidity."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "Bad things keep happening to me, like I have bad luck or something."
    },
    {
      character: "Red Forman",
      quote:
        "Son, you don't have bad luck. The reason bad things happen to you? is because you're a dumbass."
    }
  ],
  [
    { character: "Red Forman", quote: "Forman, party of two." },
    {
      character: "Restaurant Hostess",
      quote: "Okey dokey, that'll be about two hours."
    },
    { character: "Red Forman", quote: "Here's twenty bucks." },
    {
      character: "Restaurant Hostess",
      quote: "Okay we'll have something in fifteen minutes."
    },
    {
      character: "Red Forman",
      quote: "You don't want this place to burn down twice do you?"
    },
    {
      character: "Restaurant Hostess",
      quote: "Okay we have something right now."
    },
    {
      character: "Red Forman",
      quote: "I thought so. Well, it looks like it's our lucky night."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "What are you going to put on your resume - dumbass?"
    }
  ],
  [
    {
      character: "Red",
      quote:
        "[after Eric refused to be Donna's boyfriend again]  Let me get this straight - Donna told you she wanted to get back together and you said NO?"
    },
    { character: "Eric", quote: "I said no." },
    { character: "Red", quote: "What the Hell is wrong with you?" },
    { character: "Kitty Forman", quote: "Dumbass!" },
    { character: null, quote: "[Red looks at Kitty in surprise]" },
    { character: "Eric", quote: "Look, I have my reasons, okay?" },
    { character: "Red", quote: "What the Hell could they possibly be?" },
    {
      character: "Eric",
      quote:
        "Casey dumps Donna and then Donna comes running back to ME? Look, I'm not a rebound, All right?"
    },
    {
      character: "Red",
      quote:
        "You're too PROUD to take her back? What exactly do you have to be proud OF? You're not an athelete! The only thing smart about you is your mouth! And, well... look at you!"
    }
  ],
  [
    {
      character: null,
      quote: "[Eric catches his parents having sex, and they find out]"
    },
    { character: "Kitty Forman", quote: "Red, say something." },
    { character: "Red Forman", quote: "It's more fun than it looks." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "We're all gonna go to church and we're gonna have a damn nice Sunday."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Without rules, we all might as well be up in a tree flinging our crap at each other."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "[on the new water heater]  This is the best water heater ever. God bless us, every one."
    },
    {
      character: "Red Forman",
      quote: "Do you know why Tiny Tim walked with a crutch?"
    },
    { character: "Eric", quote: "Because he had a smart mouth?" },
    { character: "Red Forman", quote: "That's right." }
  ],
  [
    { character: null, quote: "[repeated line]" },
    { character: "Red Forman", quote: "That kid's on dope!" }
  ],
  [
    { character: "Red Forman", quote: "What are you doing here?" },
    { character: "Michael Kelso", quote: "The explanation is in the note." },
    {
      character: "Red Forman",
      quote:
        "[reading the note]  Dear Red, we would like for you to give Michael thirty dollars for the game you threw out the window and broke. Signed, my parents."
    },
    {
      character: null,
      quote: "[Kitty laughs hysterically, and leaves the room]"
    },
    {
      character: "Red Forman",
      quote: "Well, you made her laugh, that's worth at least thirty bucks."
    },
    { character: null, quote: "[Gives Kelso the money]" }
  ],
  [
    { character: "Red Forman", quote: "What the hell happened?" },
    { character: "Steven Hyde", quote: "Eric made out with Laurie's friend." },
    { character: "Red Forman", quote: "Anything else?" },
    { character: "Fez", quote: "Your son is a whore." }
  ],
  [
    { character: null, quote: "[about Bob's hair]" },
    { character: "Red Forman", quote: "His head looks like a poodle's ass." }
  ],
  [
    { character: "Kitty Forman", quote: "Sex, it's not dirty." },
    { character: "Red Forman", quote: "It's not clean either." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "What the hell kind of restaurant is this? You got eight people singing happy birthday, and no one can bring my wife a damn salad."
    }
  ],
  [
    { character: "Eric", quote: "I got a B." },
    { character: "Red Forman", quote: "You couldn't get an A?" },
    {
      character: "Kitty Forman",
      quote:
        "Aww honey, don't listen to him. You did super. And Steven, you did super duper."
    },
    { character: "Eric", quote: "Why does he get a duper?" }
  ],
  [
    {
      character: "Red Forman",
      quote: "Kelso, you make Eric look like Einstein."
    },
    { character: "Eric", quote: "[sarcastically]  Gee, thank you daddy." },
    { character: "Michael Kelso", quote: '"Thank you"? Einstein was ugly.' }
  ],
  [
    { character: null, quote: "[repeated line]" },
    { character: "Red Forman", quote: "Dumbass!" }
  ],
  [
    { character: null, quote: "[referring to that Thanksgiving's events]" },
    {
      character: "Red Forman",
      quote:
        "You mean with that ungrateful daughter, our dumbass son, and that drunk foreign kid"
    },
    { character: null, quote: "[pause]" },
    { character: "Red Forman", quote: "it was better than last year." }
  ],
  [
    {
      character: "Bob Pinciotti",
      quote:
        "[Bob has come over to the Foremans place after he cut down a tree that almost fell on Red]  Red, I'm so sorry. It was an accident."
    },
    {
      character: "Red Forman",
      quote:
        "[Red's acting nice]  I thought I never say this. I'm glad you're my friend."
    },
    { character: null, quote: "[hugs Bob]" },
    {
      character: "Bob Pinciotti",
      quote: "Red, I thought I'd never say this, you smell nice."
    }
  ],
  [
    {
      character: null,
      quote: "[on buying an economy car during the oil crisis]"
    },
    {
      character: "Red Forman",
      quote:
        "The last time I was that close to a Japanese machine, it was shooting at me."
    }
  ],
  [
    { character: null, quote: "[seeing a square dance]" },
    { character: "Red Forman", quote: "It looks like 'Hee Haw' puked in here." }
  ],
  [
    {
      character: "Red",
      quote:
        "Well, we got vandals in this town. I was driving home and I saw the water tower giving me the finger."
    },
    {
      character: "Laurie Forman",
      quote: "Vandals you say? Hmm, where were you last night, Eric?"
    },
    {
      character: "Kitty Forman",
      quote: "Oh, can it, Laurie. Eric you look pale. Let me see your eyes."
    },
    { character: null, quote: "[Eric just looks down]" },
    { character: "Kitty Forman", quote: "Look at me." },
    {
      character: null,
      quote:
        "[Eric looks at Kitty, imagining her she was naked after seeing his parents having sex]"
    },
    { character: "Kitty Forman", quote: "Do you have fever?" },
    {
      character: "Red",
      quote:
        "[Eric looks at Red, and he's imagining him naked, too]  I know what you need. Right after breakfast, I want you to mow the lawn. The fresh air will do you good."
    },
    {
      character: "Kitty Forman",
      quote:
        "[the camera turns around back to Kitty. Kitty is still naked and she wipes something off her chest]  Eric, is something bothering you?"
    },
    {
      character: "Eric",
      quote:
        "[looking at Kitty and Red a couple more times]  God, make it stop!"
    },
    { character: null, quote: "[leaves the table]" }
  ],
  [
    { character: "Red Forman", quote: "Threats aren't going to work, Kitty." },
    {
      character: "Kitty Forman",
      quote:
        "Every newspaper you'll be reading, every nap you'll be taking, every football game you'll be watching, I'll be there, talking, talking, talking, talking."
    }
  ],
  [
    { character: null, quote: "[on Valentine's day]" },
    {
      character: "Kitty Forman",
      quote:
        "Oooh, look how cute you are. This is just like when you were 5, and played make believe dinner. Only this time, it won't end by Donna sitting on Eric's face."
    },
    { character: null, quote: "[Awkward pause]" },
    { character: "Kitty Forman", quote: "OK." },
    { character: null, quote: "[Kitty and Red start leaving]" },
    { character: "Red Forman", quote: "Kitty, that was bad." },
    { character: "Kitty Forman", quote: "I know, keep walking." },
    { character: null, quote: "[Kitt and Red finally leave]" },
    {
      character: "Eric",
      quote: "Oh, good, and I thought this was going to be awkward."
    }
  ],
  [
    { character: "Red Forman", quote: "This is a smoke detector." },
    { character: "Michael Kelso", quote: "Does that detect any type of smoke?" }
  ],
  [
    {
      character: "Eric",
      quote: "So, you're saying that Donna and I will be okay?"
    },
    {
      character: "Red Forman",
      quote:
        "No. All I did was scratch your mother's shoes. I didn't pull her pants down in front of God and the foreign kid."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "When used separately, women and alcohol can be a lot of fun. But if you mix them, they can turn you into a dumbass."
    }
  ],
  [
    { character: "Kitty Forman", quote: "Bed checks, here we come." },
    { character: "Red Forman", quote: "Shh, Kitty, you're warning them." },
    {
      character: "Kitty Forman",
      quote: "I'm not warning anybody. Getting closer."
    }
  ],
  [
    { character: "Bob Pinciotti", quote: "You know Red, that hurts." },
    { character: "Red Forman", quote: "So does a swift kick in the ass." },
    {
      character: "Bob Pinciotti",
      quote:
        "You know, Red, a kick in the ass isn't the solution to everything."
    },
    {
      character: "Red Forman",
      quote: "I'm afraid I'm gonna have to disagree with that, Bob."
    }
  ],
  [
    { character: "Red Forman", quote: "Penny, you're cold. Go get a sweater." },
    { character: null, quote: "[Cousin Penny leaves]" },
    {
      character: "Red Forman",
      quote: "Eric, do I have to tell you to stop staring at your cousin?"
    }
  ],
  [
    {
      character: "Eric",
      quote: "Mom, Dad, can Penny and I be alone for a minute?"
    },
    {
      character: "Red Forman",
      quote: "No. I'm not raising any flipper grandkids."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Every single Price Mart stock-boy will be trained in the Red Forman way."
    },
    {
      character: "Eric",
      quote:
        "Oh, you mean by undermining their self-esteem until they're too weak to fight back?"
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote:
        "Eric, your father and I have noticed that you've been acting very strange lately."
    },
    { character: "Red Forman", quote: "Like a hippie." },
    {
      character: "Kitty Forman",
      quote: "Is there something you want to tell us?"
    },
    { character: "Red Forman", quote: "Are you on dope? Are you?" },
    {
      character: "Kitty Forman",
      quote:
        "Because we can help get you clean. There's counseling, hospitalization..."
    },
    { character: "Red Forman", quote: "...my foot kickin' your ass." }
  ],
  [
    { character: "Red Forman", quote: "[to Fez]  Hey, Ali Baba. Close Sesame." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to Fez]  Thanks for the help. You seem to have a natural talent for handling luggage."
    }
  ],
  [
    { character: null, quote: "[Red on Laurie and Michael]" },
    {
      character: "Red Forman",
      quote:
        "This is how it starts, you know. First they're dating and having fun, and then the next thing you know, they're prying the money to pay for a wedding from my cold, dead hands."
    },
    {
      character: "Kitty Forman",
      quote:
        "Oh, honey. They're not going to get married. He'll leave her when she gets pregnant."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to Steven]  If you ever do anything like that again, I will kick your ass so hard, your nose will bleed."
    },
    { character: "Kitty Forman", quote: "And we love you." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        'If the US government decides to stick a tracking device up your ass, you say, "Thank You. And God Bless America."'
    }
  ],
  [
    { character: null, quote: "[Kitty wants to redecorate the basement]" },
    {
      character: "Red Forman",
      quote: "This is going to be expensive, isn't it?"
    },
    { character: "Kitty Forman", quote: "Maybe." },
    { character: "Red Forman", quote: "Don't you like anything cheap?" },
    { character: "Kitty Forman", quote: "I like you." }
  ],
  [
    {
      character: "Red Forman",
      quote: "Once again, an open bar spells disaster for the Foreman family."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Dear Lord, would it kill you to give the Packers a winning season? Oh, and uh, watch over my loving family, blah blah blah..."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "I believe that everyone's political opinion is valid, and worth hearing."
    },
    {
      character: "Red Forman",
      quote:
        "Well that's perfect Eric. Use that line when you run for Miss America."
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote: "Red, there are five stages of grieving"
    },
    { character: "Red Forman", quote: "Kitty, I've got two stages" }
  ],
  [
    {
      character: "Laurie Forman",
      quote: "You know Eric, hickeys lead to dirty things."
    },
    {
      character: "Red Forman",
      quote: "For God sakes. Don't let Donna suck your neck."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Damn kids today. They wouldn't know responsibility if it walked up and bit them in the ass."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Eric, I thought I told you to wash up for dinner. I know, it's difficult to hear with your head up your ass."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "I've been working since I was sixteen. I fought in two wars. Hell, I've killed people. I'm not saying that I didn't enjoy it..."
    }
  ],
  [{ character: "Red Forman", quote: "I like the sound of a beer church." }],
  [
    {
      character: "Michael Kelso",
      quote:
        "Don't freak out, but if I see a deer on the side of the road, I'm gonna shoot him."
    },
    {
      character: "Red Forman",
      quote:
        "Kelso, you fire that gun in this car, and I will pull over and kick your ass for an hour."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "[Red has a dream that he dies, and nobody shows up for his funeral]"
    },
    {
      character: "Red",
      quote: "Where is everybody? Where are all my friends?"
    },
    {
      character: "Kitty Forman",
      quote:
        "You don't have any friends, honey. I'm afraid it's a side effect of... telling people to stick it in their butts."
    }
  ],
  [
    { character: "Kitty Forman", quote: "My parents are coming tomorrow." },
    { character: "Red Forman", quote: "Oh, crap." },
    { character: "Kitty Forman", quote: "Red." },
    { character: "Donna Pinciotti", quote: "What's wrong with them." },
    { character: "Kitty Forman", quote: "It's a complicated situation." },
    { character: "Eric", quote: "Grandma yells, grandpa drinks." }
  ],
  [
    {
      character: "Red",
      quote:
        "Son, you don't have bad luck. The reason that bad things happen to you is because you're a dumbass."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Good night, sleep tight, and don't let the bedbugs put their foot in your ass."
    }
  ],
  [
    { character: "Red Forman", quote: "Oh and uh, here's a 20." },
    { character: "Laurie Forman", quote: "Will that cover for gas?" },
    {
      character: "Kitty Forman",
      quote: "Oh well, honey, give her another 10 just in case."
    },
    { character: "Eric", quote: "You know, I could use some gas money." },
    {
      character: "Red Forman",
      quote:
        "[laughs]  Yeah... and if a frog had wings, it wouldn't bump it's ass when it hops."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Do you know how they treat their criminals in Russia? First offense, five years in Siberia. Second offense, ten years. Believe you me, there is no third offense."
    },
    {
      character: "Kitty Forman",
      quote: "Unless the criminal likes to make snow angels."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "Bend your knees and lift with your legs, or else I'm going to..."
    },
    {
      character: "Eric",
      quote:
        "...kick my ass, put your foot in my ass, make my ass a hat, yeah, yeah yeah..."
    }
  ],
  [
    { character: "Bob Pinciotti", quote: "uh-heh" },
    { character: "Red Forman", quote: "OK, Bob, What is it?" },
    {
      character: "Bob Pinciotti",
      quote: "I'm just curious. What's the word on that Hyde kid?"
    },
    {
      character: "Red Forman",
      quote: "Steven? He's a little rebellious, just needs some direction."
    },
    {
      character: "Bob Pinciotti",
      quote:
        "Apparently he's getting it, Red. I kind of walked into your living room and he and Kitty were in some sort of provocative embrace. I think he's putting the moves on your wife."
    },
    {
      character: "Red Forman",
      quote: "Ooh my God. Now I've gotta kill him. Get your deer rifle, Bob."
    },
    {
      character: "Bob Pinciotti",
      quote: "H... Hold on Red. I... I... I could have been mistaken."
    },
    {
      character: "Red Forman",
      quote: "You know what you saw. Get the damn gun."
    }
  ],
  [
    { character: null, quote: "[Red on young people]" },
    {
      character: "Red Forman",
      quote:
        "Get a job, HA. It's just party all night, dance all day, and sex everywhere in between."
    }
  ],
  [
    { character: null, quote: "[Red groans]" },
    {
      character: "Eric",
      quote:
        "Well, Marlin, we've just seen the male of the herd grunt his displeasure. But what does it mean?"
    },
    {
      character: "Steven Hyde",
      quote: "The grunting indicates aggression, Jim."
    },
    { character: "Eric", quote: "Ah." },
    { character: "Red Forman", quote: "Idiots." }
  ],
  [
    { character: "Kitty Forman", quote: "How's it going?" },
    {
      character: "Red Forman",
      quote: "Real good. The foreign kid just ate something off the floor."
    }
  ],
  [
    {
      character: "Steven Hyde",
      quote: "Hey, Red. You have to sign this card for me."
    },
    { character: "Red Forman", quote: "It says you're failing gym." },
    { character: "Steven Hyde", quote: "Yeah." },
    { character: "Red Forman", quote: "Why the hell are you failing gym?" },
    { character: "Steven Hyde", quote: "Because I wouldn't wear shorts?" },
    { character: "Red Forman", quote: "Why not?" },
    { character: "Steven Hyde", quote: "Would you wear shorts?" },
    { character: null, quote: "[Red signs the card]" }
  ],
  [
    { character: "Kitty Forman", quote: "I want to have a baby." },
    {
      character: "Red Forman",
      quote:
        "You still got Eric. He's kinda like a baby. I can make him cry if you want to."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "Is that kid from not America still here?"
    }
  ],
  [
    { character: null, quote: "[about Bob and Midge]" },
    {
      character: "Red Forman",
      quote:
        "What the hell kind of a world are we living in? 'Hey, let's date other people.' 'Hey, let's date other people, but ditch them and do it in a car.' In my day, we called them degenerates, and we STONED them."
    }
  ],
  [
    { character: "Red", quote: "Earl, just get to work." },
    {
      character: "Earl",
      quote:
        "Oh, sure. Right after my cup of joe. I'm useless without my coffee."
    },
    { character: null, quote: "[Earl runs out]" },
    {
      character: "Eric",
      quote: "Then, I guess he hasn't had coffee in years, huh?"
    }
  ],
  [
    { character: null, quote: "[Eric just announced his plans to move away]" },
    { character: "Red Forman", quote: "I'm proud of you." },
    {
      character: "Kitty Forman",
      quote: "And that goes double for... what the hell did you say?"
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Steven, I've come to think of you as a son. So I want to give you some honest, heart-felt advice. Get your head out of your ass."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "[to Hyde and Fez after finding pot stashed in the basement]  You morons just hung a vacancy sign on your asses and my foot's looking for a room!"
    }
  ],
  [
    { character: "Red Forman", quote: "Why am I doing this?" },
    {
      character: "Kitty Forman",
      quote:
        "Because I want to force some good cheer into your clogged arteries and into your cranky heart."
    },
    { character: "Steven Hyde", quote: "Yeah, it's like Christmas Drano." }
  ],
  [
    {
      character: "Red",
      quote:
        "[after having a bucket of oatmeal dumped on his head]  What the hell is going on?"
    },
    {
      character: "Eric",
      quote:
        "Dad, it was just a prank that went wrong. Horribly... *horribly* wrong."
    },
    {
      character: "Red",
      quote:
        "Really? Well I have a prank too. One where my foot *doesn't* plow through your ass! Let's hope it doesn't go horribly, *horribly* wrong!"
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote: "Wait. Are you saying you LIKE Randy?"
    },
    {
      character: "Red Forman",
      quote:
        "No... I'm just saying I don't hate him as much as I hate most people."
    }
  ],
  [
    {
      character: null,
      quote: "[Eric puts boxes on a dolly. He whistles. Red works at his desk]"
    },
    {
      character: "Red",
      quote:
        "You know what the great thing about whistling is? It's that you can stop whistling!"
    },
    { character: "Eric", quote: "Oh. Sorry." },
    {
      character: null,
      quote: "[not whistling, he puts another box on the dolly]"
    },
    { character: "Red", quote: "Hey, bend at the knees or else I'll..." },
    {
      character: "Eric",
      quote:
        "Kick my ass, put your foot in my ass, make my ass a hat, yeah, yeah, yeah."
    },
    {
      character: "Red",
      quote:
        "Geez, and I didn't think you were listening. Oh, and I need you to re- sticker the clock radios. They're on sale."
    },
    {
      character: "Eric",
      quote: "Yeah, I saw the flyer. I already took care of it."
    },
    { character: "Red", quote: "Really? Well, way to take initiative." },
    {
      character: "Eric",
      quote: '"Way to take initiative... "? What are you up to?'
    },
    { character: "Red", quote: "Nothing. I just think you did a good job." },
    { character: "Eric", quote: "Okay. But I'm watching you." },
    { character: "Red", quote: "Stop being weird." }
  ],
  [
    {
      character: "Red Forman",
      quote: "[Red and Eric are in the car almost late for work at Pricemart]"
    },
    { character: null, quote: "[to himself]" },
    { character: "Red Forman", quote: "I love my job." },
    { character: "Eric", quote: "I love you too, Dad." },
    { character: "Red Forman", quote: "Oh, Eric, stop being weird." },
    { character: "Eric", quote: "Thanks, Dad!" }
  ],
  [
    { character: "Eric", quote: "Hey dad. You coming back inside?" },
    {
      character: "Red Forman",
      quote:
        "Eric, I love your grandmother very much. I just can't talk to her or spend any time with her."
    },
    {
      character: "Eric",
      quote: "Did I mention that I'm in there rubbing her feet?"
    },
    {
      character: "Red Forman",
      quote: "Eric, sometimes you got to play through the pain."
    },
    { character: "Eric", quote: "What?" },
    { character: "Red Forman", quote: "I don't know. Just, get back in there." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "I'm glad he's in prison for bribery. People like him give a bad name to Republicans."
    },
    {
      character: "Eric",
      quote: "Yeah, all the honest ones, like Richard Nixon."
    },
    { character: null, quote: "[Everybody moves away from Eric]" },
    { character: "Red Forman", quote: "What did you just say?" },
    {
      character: "Eric",
      quote: "Uhh... I said Nixon was framed, and Kennedy was a commie?"
    },
    { character: "Red Forman", quote: "Damn right." }
  ],
  [
    {
      character: "Red Forman",
      quote: "So, you mean, we met by you bumping into my ass?"
    },
    { character: "Kitty Forman", quote: "I guess so." },
    {
      character: "Red Forman",
      quote:
        "Ok, but if Eric asks, I punched out a marine, defending your honor."
    },
    {
      character: "Kitty Forman",
      quote: "And, I wasn't drunk, I was reading for the blind."
    },
    { character: "Red Forman", quote: "Deal." }
  ],
  [
    { character: null, quote: "[Jackie's dad got arrested]" },
    {
      character: "Red Forman",
      quote:
        "Look, Jackie. I don't what to say except... your dad's rich, isn't he?"
    },
    { character: "Jackie Burkhardt", quote: "Yeah." },
    {
      character: "Red Forman",
      quote: "Well, you can use that to get him out of prison."
    },
    {
      character: "Jackie Burkhardt",
      quote:
        "Oh, my god. That's such a good idea. I'll do that. You know, it's true. In this country, the rich get treated better than most people."
    },
    { character: "Red Forman", quote: "Yeah." },
    { character: null, quote: "[sarcastically]" },
    {
      character: "Red Forman",
      quote: "I'm so glad I took some shrapnel to make that possible..."
    },
    { character: "Jackie Burkhardt", quote: "Me too." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to Eric]  Happy Birthday. You know, the lawn's not gonna cut itself."
    }
  ],
  [
    { character: null, quote: "[Red watches Tv]" },
    {
      character: "Red Forman",
      quote: "Aw, Gilligan screwed it up. Why don't they just kill him?"
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote:
        "[at church fundraiser]  Ok, I have jobs for everyone. Red, you can run the raffle."
    },
    { character: "Red Forman", quote: "I'm your man." },
    { character: "Kitty Forman", quote: "Don't yell at the customers." },
    { character: "Red Forman", quote: "I'm... kinda your man." },
    { character: "Kitty Forman", quote: "And smile." },
    { character: "Red Forman", quote: "You need another man." }
  ],
  [
    {
      character: "Red Forman",
      quote: "[looking in bag of pot]  Is this what I think it is?"
    },
    {
      character: "Michael Kelso",
      quote: "If you mean paprika, then yes, sir!"
    },
    { character: "Kitty Forman", quote: "Honey, paprika is red." },
    {
      character: "Michael Kelso",
      quote: "If you mean green paprika, then yes, sir!"
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Eric, if your mother wants you and Archie and Jughead to help her, then you'll help her."
    },
    { character: "Michael Kelso", quote: "[to Steven]  You're Jughead." },
    {
      character: "Steven Hyde",
      quote: "You're so Jughead, its not even debatable."
    },
    { character: "Michael Kelso", quote: "You are so..." },
    { character: null, quote: "[Steven punches Michael]" },
    { character: "Red Forman", quote: "Steven, stop hitting Jughead." }
  ],
  [
    {
      character: "Red Forman",
      quote: "Why is our house always infested with kids?"
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to Hyde]  You know all that rent money you've been giving us? I've been putting it all in a bank account for your college fees... or bail."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Bud, being a teenager is like being in a war. One minute, you're crawling around blind. Next minute, you get your foot blown off."
    },
    {
      character: "Bud Hyde",
      quote:
        "I wouldn't know anything about that. Never been to war. I was a conscientious."
    },
    { character: null, quote: "[Red looks at him menacingly]" },
    { character: "Bud Hyde", quote: "Uhh... Canadian." }
  ],
  [
    {
      character: "Leo",
      quote: "Hi, Red. Would you give these to Kitty, please?"
    },
    { character: null, quote: "[hands Red flowers]" },
    { character: "Leo", quote: "Oh, and tell her I love her. Thanks." },
    { character: null, quote: "[leaves]" },
    {
      character: "Red Forman",
      quote:
        "Oh, Kitty. These are for you. Looks like you got a date with a stoner."
    },
    { character: null, quote: "[Red and Kitty laugh]" },
    {
      character: "Kitty Forman",
      quote:
        '[takes flowers and reads attached note]  "Roses are red, violets are blue. Milk, eggs, coffee."'
    }
  ],
  [
    { character: null, quote: "[Kitty has menopause]" },
    {
      character: "Red Forman",
      quote: "[to Eric]  Quick, I need a wet towel and a Bloody Mary."
    },
    { character: null, quote: "[Eric stares at him]" },
    {
      character: "Red Forman",
      quote:
        "Your mother is talking about adopting a communist orphan. HURRY, DAMMIT."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Michael Kelso scored higher than you on the test? This is the kid I saw super-glue his hand to his face."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "[to Eric]  This is the worst thing you've ever done."
    },
    { character: "Bob Pinciotti", quote: "[to Donna]  You too." },
    {
      character: "Red Forman",
      quote:
        "You're gonna drive Donna home, and then you're gonna wait for me. That's an order."
    },
    {
      character: "Bob Pinciotti",
      quote:
        "Ditto... Aww, come here. I can't stay mad at you with that cute face."
    },
    { character: null, quote: "[hugs her; Eric looks at Red with open arms]" },
    { character: "Red Forman", quote: "Get your ugly ass in the car." }
  ],
  [
    { character: null, quote: "[the kids want to throw a party]" },
    { character: "Red Forman", quote: "Why here? Why is it always here?" },
    {
      character: "Kitty Forman",
      quote: "Well, what do you want them to do? Throw the party in the street?"
    },
    {
      character: "Red Forman",
      quote: "Yes. They puke in the street, the city cleans it up."
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote:
        "You know, maybe Eric's test score is a blessing. It will be a good story when he's a senator."
    },
    {
      character: "Red Forman",
      quote: "Senator? The word you're looking for is JANITOR."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "You know, Steven, you're a smart guy. You really should go to college. You're coming with us, this weekend."
    },
    {
      character: "Steven Hyde",
      quote: "You don't trust me alone in the house, do you?"
    },
    { character: "Red Forman", quote: "See how smart you are?" }
  ],
  [
    { character: "Red Forman", quote: "What's going on?" },
    {
      character: "Michael Kelso",
      quote: "Nothing. Just a classic case of Hand-Stuck-In-Vase."
    },
    {
      character: "Red Forman",
      quote:
        "Well, if you don't get your hand out of there, you're gonna have a classic case of Foot-Stuck-In-Ass."
    }
  ],
  [
    { character: null, quote: "[Laurie moves out]" },
    {
      character: "Red Forman",
      quote:
        "Aw, Kitty, you think that she's all grown up. But there's still a lot that she doesn't know. I mean, what if her place doesn't have a deadbolt, or a smoke detector?"
    },
    {
      character: "Kitty Forman",
      quote: "Red, you don't have to baby her. She's twenty."
    },
    {
      character: "Red Forman",
      quote: "You're never too old to burn to death in a fire."
    }
  ],
  [
    {
      character: null,
      quote: "[Eric likes Stacey at Price Mart, but Stacey likes Red]"
    },
    { character: "Eric", quote: "Did Stacey actually use the word 'freak'?" },
    {
      character: "Red Forman",
      quote:
        "Why don't you just let it go? Maybe the reason girls don't like you, is because you just don't let things go. Stacey was saying just that thing today."
    }
  ],
  [
    { character: "Bob Pinciotti", quote: "You're my best friend!" },
    { character: "Red", quote: "No I'm not!" }
  ],
  [
    { character: null, quote: "[Eric and Red are hunting]" },
    {
      character: "Red Forman",
      quote:
        "I want you to forget that I'm your father for a minute. I want you to tell me exactly what you think of me."
    },
    {
      character: "Eric",
      quote: "How about I do this when you don't have a gun?"
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "Let's not talk about it in front of the boy."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Eric's old enough to hear this kind of talk. Eric, say your job got transferred to Guada-Who-The-Hell-Cares. Would you vote for the guy that did that?"
    },
    {
      character: "Kitty Forman",
      quote: "Oh, Red, Ford didn't take away your job. He took Nixon's."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "Hey, dad, um, I was wondering if you could show me, like, a few fighting moves."
    },
    { character: "Red", quote: "Who you planning to fight?" },
    { character: "Eric", quote: "David Milbank." },
    {
      character: "Bob Pinciotti",
      quote: "David Milbank? He's got scoliosis and asthma."
    },
    { character: null, quote: "[pause]" },
    { character: "Bob Pinciotti", quote: "You could take him." },
    {
      character: "Red",
      quote:
        "Oh, come on now, Eric. Why don't you, uh, beat up Kelso? I don't work for his dad."
    },
    { character: "Eric", quote: "He's making a move on Donna." },
    {
      character: "Bob Pinciotti",
      quote:
        "Oh, no. No. No. Donna's not going near that pretzel boy! No. No. You gotta nip this in the bud, Eric."
    },
    {
      character: "Red",
      quote: "All right. All right. The bridge of the nose is very vulnerable."
    },
    { character: "Bob Pinciotti", quote: "Oh. Oh! And hit him with a banjo!" },
    { character: "Red", quote: "[pauses and stares at Bob]  A banjo, Bob?" },
    { character: "Bob Pinciotti", quote: "What? I'm helping!" },
    { character: "Red", quote: "Where's he gonna get a banjo?" },
    {
      character: "Bob Pinciotti",
      quote:
        "I don't know! But I saw a guy get hit with a banjo once, and he went down."
    },
    {
      character: "Red",
      quote:
        "[pauses and stares at Bob again]  Hitting a guy with a banjo, is dirty."
    },
    { character: null, quote: "[to Eric]" },
    { character: "Red", quote: "You wanna knee him in the groin." },
    {
      character: "Bob Pinciotti",
      quote: "You can hit him in the groin with a banjo."
    }
  ],
  [
    { character: null, quote: "[At Breakfast]" },
    { character: "Eric", quote: "Hey, leggo my Eggo." },
    { character: "Red Forman", quote: "Hey, leggo my foot in your ass." }
  ],
  [
    { character: "Kitty Forman", quote: "Where did you learn your parenting?" },
    { character: "Red Forman", quote: "Korea." }
  ],
  [
    { character: null, quote: "[Kitty has just come home from work]" },
    {
      character: "Kitty Forman",
      quote: "Here, honey. I brought you a lollipop."
    },
    { character: "Eric", quote: "Mom, I'm 17." },
    {
      character: "Kitty Forman",
      quote: "I know. That's why I also brought you condoms."
    },
    { character: "Eric , Red", quote: "Ehhhhhhh." },
    {
      character: "Kitty Forman",
      quote:
        "You need to protect yourself. Today I saw a 16-year-old give birth, and I don't want that type of thing to happen to you."
    },
    { character: "Eric", quote: "Well, this couldn't be more uncomfortable." },
    { character: null, quote: "[gets up and leaves]" },
    {
      character: "Kitty Forman",
      quote: "Oh, honey. Don't forget your condoms."
    },
    { character: "Eric", quote: "I was wrong." }
  ],
  [
    {
      character: "Kitty Forman",
      quote: "Red, Bob was very upset when he left here."
    },
    {
      character: "Red Forman",
      quote: "Bob's always upset. He's a little girl in big boy pants."
    }
  ],
  [
    {
      character: "Eric",
      quote: "I don't think I've ever seen Mom so mad. Have you?"
    },
    { character: "Red Forman", quote: "Not since she's stopped drinking." },
    { character: "Eric", quote: "What?" },
    { character: "Red Forman", quote: "Nothing." }
  ],
  [
    { character: null, quote: "[Red on Kitty's parents]" },
    {
      character: "Red Forman",
      quote:
        "You know, I was hoping when they'd move to Arizona, they would get lost in the desert."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "You know, maybe instead of finding another idiot to hang out with, you might think about unloading a few."
    },
    { character: null, quote: "[to Hyde]" },
    { character: "Red Forman", quote: "Like you." },
    { character: null, quote: "[to Kelso]" },
    { character: "Red Forman", quote: "And you." },
    { character: null, quote: "[to Eric]" },
    { character: "Red Forman", quote: "And you." },
    { character: "Eric", quote: "I'm your son." },
    { character: "Red Forman", quote: "Hey, I treat everyone as equals." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "Kitty, I think we should rethink our 'Don't throw Leo out on his ass policy.'"
    },
    {
      character: "Kitty Forman",
      quote: "No, no. Leo stuck around because he cares about Steven."
    },
    { character: "Leo", quote: "...and plus I can't find my shoes, man." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "I say good riddance. That cat was always making a mess on my lawn and going through my garbage."
    },
    {
      character: "Laurie Forman",
      quote: "Yeah. But now we have Hyde to do that."
    },
    {
      character: "Steven Hyde",
      quote:
        "Oh yeah, Laurie? And what exactly do you do? Oh, yeah - the Packers."
    }
  ],
  [
    { character: "Kitty Forman", quote: "Steven's father is in town?" },
    {
      character: "Laurie Forman",
      quote: "Yeah, right. Like he even knows who his father is."
    },
    { character: null, quote: "[gets up and leaves]" },
    { character: "Red Forman", quote: "Do you know anything about this?" },
    { character: "Eric", quote: "Yeah. She's a bitch." }
  ],
  [
    {
      character: "Red Forman",
      quote: "A bond between a father and son is subtle and complex-"
    },
    { character: null, quote: "[to Eric]" },
    { character: "Red Forman", quote: "You wipe that smirk off your face." }
  ],
  [
    { character: null, quote: "[on taking care of Red's parking ticket]" },
    {
      character: "Nina",
      quote: "I can't take care of this. You were parked in a fire zone."
    },
    { character: "Red Forman", quote: "I was buying some milk." },
    { character: "Nina", quote: "But what if there was a fire?" },
    { character: "Red Forman", quote: "Then I'd pour my milk on it." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "I'm... sorry that I took your money out of your little box, when I feed you and clothe you and put a roof over your head. Sorrrrry."
    }
  ],
  [
    { character: null, quote: "[on Bob's barbecue]" },
    {
      character: "Red Forman",
      quote:
        "Well isn't that a surprise. A National Guardsman serving up chicken."
    }
  ],
  [
    { character: null, quote: "[on Bob]" },
    {
      character: "Red Forman",
      quote: "I didn't want to insult him by offering him some nothing job."
    },
    { character: "Kitty Forman", quote: "Oh Red, you insult him everyday." },
    {
      character: "Red Forman",
      quote: "But that's different. He thinks I'm kidding."
    }
  ],
  [
    { character: null, quote: "[Red has just won at craps]" },
    { character: "Red Forman", quote: "I'm the richest man in Church." }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "I don't see why we have to spend the night. It only took me ten minutes to catch up with everybody at my reunion."
    },
    {
      character: "Kitty Forman",
      quote:
        'Red, standing in the corner by yourself muttering "Dumbass" at everyone doesn\'t count as "catching up".'
    }
  ],
  [
    { character: "Bull", quote: "So, I hear your plant's closing down." },
    {
      character: "Red Forman",
      quote:
        "Yeah, you can't compete with those damn foreign imports. You know, if I had seen this coming, I would've shot a little straighter during the war."
    }
  ],
  [
    { character: null, quote: "[after Red insults Fez, Eric, and Kelso]" },
    {
      character: "Kelso",
      quote: "Hey, hold on a second. Am I the pretty boy moron?"
    },
    { character: "Red", quote: "Yes." },
    { character: "Kelso", quote: "Cool, because that's the best one." }
  ],
  [
    {
      character: "Red",
      quote:
        "Donna just came through hear looking pretty upset. You wouldn't happen to know anything about that, would you?"
    },
    {
      character: "Eric",
      quote:
        "No. I mean, she was fine just a minute ago when we were down... oh, you already know, don't you?"
    },
    { character: "Red", quote: "Of course I know." }
  ],
  [
    { character: null, quote: "[Kitty wants Hyde to move in with them]" },
    { character: "Red", quote: "For God's sake, Kitty, I'm not Santa Claus." },
    {
      character: "Kitty Forman",
      quote:
        "Well, thank God you're not Santa Claus, Red. You scare the hell out of children."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "If one more person tells me to 'shut it'..."
    },
    { character: "Kitty Forman", quote: "You might actually 'shut it'?" }
  ],
  [
    {
      character: "Red Foreman",
      quote: "Earl, I didn't make you too dumb to flip burgers. It's God fault."
    }
  ],
  [
    { character: "Red", quote: "Look at this. First day of deer season." },
    { character: "Eric", quote: "Rabbit season." },
    { character: "Steven Hyde", quote: "Duck season." },
    { character: "Eric", quote: "Rabbit season." },
    { character: "Steven Hyde", quote: "Duck seaon." },
    { character: "Eric", quote: "Rabbit season" },
    { character: "Steven Hyde", quote: "Duck season." }
  ],
  [
    { character: null, quote: "[Red complains about the neighbors' dog]" },
    {
      character: "Red",
      quote:
        "That thing was always messing in my yard and going through my trash."
    },
    { character: "Laurie Forman", quote: "Now we have Hyde for that." },
    {
      character: "Steven Hyde",
      quote:
        "Oh yeah Laurie, and what exactly do you do? Oh that's right, the Packers."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "[Eric lied to his parents about staying at Fez's and Red has found out]  Okay, the reason I'd lie about something like that is..."
    },
    {
      character: "Red Forman",
      quote: "What happened? You pissed Donna off and she locked you out?"
    },
    { character: "Eric", quote: "Donna? No." },
    {
      character: "Red Forman",
      quote: "Oh. Well you pissed me off so I'm locking you out."
    }
  ],
  [
    { character: null, quote: "[flashback to twenty years earlier]" },
    {
      character: "Red Forman",
      quote:
        "It's just that Frank is getting on my nerves. He's just... he's an ass. And he's dumb. He's a..."
    },
    { character: null, quote: "[spotlight on Red; celestial chorus plays]" },
    { character: "Red Forman", quote: "... dumbass." }
  ],
  [
    {
      character: "Eric",
      quote: "Dad just got sued by a co-worker for wrongful termination."
    },
    {
      character: "Kitty Forman",
      quote:
        "Well if the news is so unpleasant I'm ignoring it. Here, have a cupcake."
    },
    {
      character: "Eric",
      quote:
        "Mom, did you hear what I said? Dad is getting sued for wrongful termination by Earl."
    },
    {
      character: "Kitty Forman",
      quote:
        "Earl? He didn't do his job, he was always late and he was a complete dumbass... person."
    },
    { character: "Eric", quote: "Mom, you said ass." },
    {
      character: "Kitty Forman",
      quote: "So did you, now give me back the cupcake."
    },
    {
      character: "Eric",
      quote: "Dad is gonna freak. I don't know how to tell him."
    },
    { character: null, quote: "[Red enters the house]" },
    { character: "Red Foreman", quote: "Tell me what?" },
    { character: "Eric", quote: "Oh, mom said the ass word." },
    { character: "Kitty Forman", quote: "Give me that." },
    {
      character: "Eric",
      quote:
        "You know what, mom, I got this one. Whoever here is not being sued by Earl for illegal termination, raise your hand."
    }
  ],
  [
    {
      character: "Eric",
      quote:
        "You know what dad if I am still working at pricemart at your... older kill me."
    },
    { character: "Red Foreman", quote: "You don't have to ask twice son." },
    { character: "Eric", quote: "That's my daddy." },
    { character: "Red Foreman", quote: "Come on let's make fun of mum's hair." }
  ],
  [
    { character: null, quote: "[about Hyde's house]" },
    { character: "Kitty Forman", quote: "He really shouldn't be here." },
    {
      character: "Red",
      quote:
        "It's not so bad. Compared to Korea, this is Shangri-La wrapped in happy-fun candy."
    }
  ],
  [
    {
      character: null,
      quote: "[seeing Donna and Eric cavorting on the kitchen table]"
    },
    { character: "Red", quote: "Damn it. That's where I eat dinner." }
  ],
  [
    {
      character: "Red",
      quote:
        "Kitty, why is it we always do what you want to do and never do what I want to do?"
    },
    { character: "Kitty Forman", quote: "It's in the Bible." }
  ],
  [
    {
      character: "Red",
      quote: "Well, I'd like to help, but not as much as I'd like not to."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "Steven, you're 18 now. It's time to start being a man. And the first rule to being a man is you gotta spend your life doing crap you don't wanna do."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "So you just be grateful that your Dad doesn't yell at you 24-hours-a-day! And don't give me that look, because this isn't yelling! When I yell, you'll know it!"
    }
  ],
  [
    {
      character: "Red",
      quote: "Hey, I go to church... just not during televised sporting events."
    }
  ],
  [
    {
      character: "Red",
      quote: "Responsible people don't go around getting their nipples twisted."
    },
    {
      character: "Eric",
      quote: "See, when you say it, though, it just sounds weird."
    }
  ],
  [
    {
      character: "Earl",
      quote:
        "Hey, guys. Sorry I'm late, Red. My dog was hit by a car, and I had to rush him to the vet."
    },
    {
      character: "Red",
      quote:
        "Earl, he was hit on Tuesday, too. I gotta say, that's one dumb dog."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "Eric, if you don't want to wear your ass for a hat, you'll get up here, pronto!"
    },
    {
      character: "Donna",
      quote: "You better go. You know how that ass- hat screws up your hair."
    }
  ],
  [
    { character: "Red", quote: "Ah, my job, and my little girl!" },
    { character: "Eric", quote: "Ok, so I'm here too, dad." },
    {
      character: "Red",
      quote:
        "Oh, and uh, speaking of you, I told Millbank's son David to come by and see you."
    },
    { character: "Eric", quote: "David Millbank? Dad, I hated that guy." },
    {
      character: "Red",
      quote:
        "Yeah, well, I hate his dad! But, I smile like hell whenever I see him!"
    }
  ],
  [
    {
      character: null,
      quote:
        "[Red has a heart attack when he learns that Fez and Laurie got married]"
    },
    {
      character: "Red",
      quote: "[to Kitty]  If I don't make it, kill the foreign kid."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[on Kitty's new health food diet]  Oh, come on! This isn't food! This is what food eats!"
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote: "Here you are, Red. Breakfast, egg whites only."
    },
    {
      character: "Red Forman",
      quote:
        "But the yellow part's the baby bird. That's the part I want to eat!"
    }
  ],
  [
    {
      character: "Steven Hyde",
      quote: "Kelso was right. *Everyone's* trying it."
    },
    {
      character: "Red Forman",
      quote:
        "[holding a can of whipped cream]  I'm telling you, this stuff's just isn't for cakes. It's great all by itself."
    },
    { character: null, quote: "[sprays some in his mouth]" },
    { character: "Red Forman", quote: "Wait, wait, wait." },
    { character: null, quote: "[sprays some on top of his head]" },
    {
      character: "Red Forman",
      quote:
        "Look at me. I'm whipped cream head. Fear me! All fear whipped cream head!"
    }
  ],
  [
    {
      character: "Kitty Forman",
      quote:
        "[Bob just gave Red a pair of shoes]  Oh, look. He gave you shoes. What do you say, Red?"
    },
    { character: "Red Forman", quote: "What the hell is wrong with you?" }
  ],
  [
    {
      character: "Kitty Forman",
      quote: "Now Eric's leaving. What am I supposed to do?"
    },
    {
      character: "Red Forman",
      quote: "Well, there's a car show in Kenosha this weekend."
    },
    {
      character: "Kitty Forman",
      quote:
        "A car show? I don't want to go to a goddamn car show in fucking Kenosha! I want 3 more fucking months with my baby boy! I can't do that now because of your bullshit! Way to go, dumbass!"
    }
  ],
  [
    {
      character: "Mitch Miller",
      quote: "Eric, I didn't take your action figure."
    },
    { character: "Red Forman", quote: "[walks by]  Doll." },
    { character: "Eric", quote: "ACTION FIGURE." }
  ],
  [{ character: "Red Forman", quote: "Don't sass me, Tarzan!" }],
  [
    {
      character: "Red Forman",
      quote: "Shoes are an inappropriate gift to give to a man."
    },
    {
      character: "Kitty Forman",
      quote:
        "How about when you joined the army, another man issued your boots?"
    },
    { character: "Red Forman", quote: "So he gave me a gun so I let it go." },
    {
      character: "Kitty Forman",
      quote: "Why can't you just accept the shoes because Bob is your friend?"
    },
    {
      character: "Red Forman",
      quote:
        "Kitty you don't understand. We don't give each other presents. We ignore each other."
    }
  ],
  [
    {
      character: "Red Forman",
      quote: "Bob, that's my stuff. You put the hell back my stuff."
    },
    {
      character: "Bob Pinciotti",
      quote: "Sorry, Red, it was in my part of the garage."
    },
    { character: "Kitty Forman", quote: "What's he talking about?" },
    { character: "Red Forman", quote: "Kitty, let me talk to him." },
    { character: "Red Forman", quote: "Bob, get the hell out of my garage!" },
    { character: "Bob Pinciotti", quote: "[upset]  Fine!" }
  ],
  [
    {
      character: "Kitty Forman",
      quote:
        "[to Red and Eric]  Now stop fighting and help me do this crossword. I need a 4-letter word for disappointment"
    },
    { character: "Red Forman", quote: "Eric." },
    { character: null, quote: "[glares at Eric]" },
    { character: "Kitty Forman", quote: "[looks up nervously]  It fits..." }
  ],
  [
    { character: "Steven Hyde", quote: "Oh, my God... what have I done?" },
    {
      character: "Red Forman",
      quote:
        "You've married a Vegas stripper. Congratulations. When Eric calls... you're the town dumbass."
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[lecturing Kitty about smoking pot]  Kitty, when we got married, we took a vow to be together through sickness and health, but nobody said anything about what to do if your wife turns into a dope fiend!"
    }
  ],
  [
    {
      character: "Red Forman",
      quote:
        "[to FES]  If you don't shut up I will put you in a crate, take you down to the port, and trade you for a years worth of bananas."
    }
  ],
  [
    {
      character: "Red",
      quote:
        "[Red's response to seeing Eric's roller disco uniform]  Nice shorts, Rainbow!"
    }
  ]
];
